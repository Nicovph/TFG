...
_AUTH_ERROR_FRAGMENT = "auth-error"
...
def _build_frontend_auth_return_url(
    ...
    *,
    ...
    Args:
        ...
        authentication_failed: Whether to include a generic non-sensitive
            fragment that lets React show the login error screen.
    ...
    # If in development mode, it redirects to the current request origin with build_absolute_uri.
    return_url = configured_url or request.build_absolute_uri("/")

    _validate_frontend_auth_return_url(return_url)

    if authentication_failed:
        return f"{return_url}#{_AUTH_ERROR_FRAGMENT}"
...
def _redirect_to_frontend_auth_status(
    ...
    *,
    ...
    Args:
        request: The Django request that reached the callback endpoint.
        authentication_failed: Whether React should show a generic login error.
